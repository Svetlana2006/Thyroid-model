"""
External Validation Script for Kaggle Dataset (tingzen/thyroid-for-pretraining)
...
OUTPUT_DIR = Path("outputs/final_model/evaluation/thyroid_for_pretraining")
...
def get_predictions(model, loader, device, seed_idx):
    ...
    # Adding seed index to the progress bar so it's clear it's progressing through the 5 seeds!
    ...
    for obj_id, data in preds.items():
        final_preds[obj_id] = {
            "label": data["label"],
            "scale_logits": [np.mean(img_logits) for img_logits in data["logits"]] # Mean across patient's images per scale
        }
    return final_preds
...
def generate_outputs(preds_dict, metrics, y_true, y_pred_prob):
    ...
    plt.plot([0, 1], [0, 1], 'k--')
    plt.title(f"Thyroid for Pretraining ROC Curve")
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.legend()
    plt.savefig(FIG_DIR / "Thyroid_Pretraining_ROC.png", bbox_inches='tight')
    plt.close()
    ...
    plt.title(f"Thyroid for Pretraining PR Curve")
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.legend()
    plt.savefig(FIG_DIR / "Thyroid_Pretraining_PR.png", bbox_inches='tight')
    plt.close()
    ...
    plt.title("Thyroid for Pretraining Confusion Matrix")
    plt.ylabel('True')
    plt.xlabel('Predicted')
    plt.savefig(FIG_DIR / "Thyroid_Pretraining_CM.png", bbox_inches='tight')
    plt.close()

    # CSV Predictions
    ids = sorted(list(preds_dict.keys()))
    with open(OUTPUT_DIR / "thyroid_for_pretraining_predictions.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    with open(OUTPUT_DIR / "thyroid_for_pretraining_error_analysis.txt", "w") as f:
    ...
    # Evaluation Report
    report = "# Thyroid for Pretraining Dataset Evaluation (Patient Level)\n\n"
    for k, v in metrics.items():
        if isinstance(v, float): report += f"**{k}:** {v:.4f}\n"
        elif isinstance(v, list): report += f"**{k}:** {[round(x, 4) for x in v]}\n"
        else: report += f"**{k}:** {v}\n"
    with open(OUTPUT_DIR / "thyroid_for_pretraining_report.md", "w") as f: f.write(report)

def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    kaggle_path = kagglehub.dataset_download('tingzen/thyroid-for-pretraining')
    ds = KaggleThyroidDataset(kaggle_path)
    ...
    print("\n" + "="*50)
    print("KAGGLE EXTERNAL VALIDATION RESULTS (PATIENT-LEVEL)")
    print("="*50)
    print(f"Ensemble TTA AUROC : {metrics['AUROC']:.4f}")
    print(f"95% Confidence Int : {metrics['95% CI']}")
    print(f"PR-AUC             : {metrics['PR-AUC']:.4f}")
    print("="*50)
    print(f"Thyroid for Pretraining evaluation complete. Artifacts saved to {OUTPUT_DIR}")
