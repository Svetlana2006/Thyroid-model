...
Evaluates the frozen 5-seed MultiLevelSwin ensemble on the target dataset.
"""
...
class KaggleThyroidDataset(Dataset):
    def __init__(self, data_root: str, transforms: list):
        ...
        # Recursively search for folders that define class names
...
def main():
    ...
    if len(dataset) == 0:
        print("No samples found. Check if the dataset uses standard 'benign'/'malignant' or '0'/'1' folder names.")
        return
    ...
    print("\n" + "="*50)
    print("KAGGLE EXTERNAL VALIDATION RESULTS")
    print("="*50)
    print(f"Ensemble TTA AUROC : {ensemble_auc:.4f}")
    print(f"95% Confidence Int : [{lower:.4f}, {upper:.4f}]")
    print("="*50)
    
    out_file = Path(args.models_dir) / "kaggle_pretrain_results.json"
    if Path(args.models_dir).exists():
        with open(out_file, "w") as f:
            json.dump({
                "dataset": "tingzen/thyroid-for-pretraining",
                "samples": len(targets),
