"""
External Validation Script for TN3K Dataset
Evaluates the frozen 5-seed MultiLevelSwin ensemble on the haifan-gong/TN3K huggingface dataset.
"""
...
# ── HuggingFace TN3K Dataset Loader ───────────────────────────────────────────
class TN3KDataset(Dataset):
    def __init__(self, hf_dataset, transforms: list):
...
def main():
    ...
    print("Downloading/Loading TN3K dataset from HuggingFace...")
    hf_ds = load_dataset("haifan-gong/TN3K", split="test")
    dataset = TN3KDataset(hf_ds, TTA_TRANSFORMS)
    print(f"Loaded {len(dataset)} samples from TN3K test split.")
