"""
External Validation Script for Guangxi Mendeley Dataset
Evaluates the frozen 5-seed MultiLevelSwin ensemble on the multimodal dataset (Ultrasound images only).
"""
...
OUTPUT_DIR = Path("outputs/final_model/evaluation/guangxi")
...
class GuangxiDataset(Dataset):
    def __init__(self, data_root: str):
        self.samples = []
        
        # Paths for Ultrasound only (no Pathomics)
        benign_dir = os.path.join(data_root, "Ultrasound images of benign thyroid lesions")
...
def get_predictions(model, loader, device, seed_idx):
    ...
    for obj_id, data in preds.items():
        final_preds[obj_id] = {
            "label": data["label"],
            "scale_logits": [img_logits[0] for img_logits in data["logits"]] # 1 image per case
        }
    return final_preds
...
def generate_outputs(preds_dict, metrics, y_true, y_pred_prob):
    ...
    plt.plot([0, 1], [0, 1], 'k--')
    plt.title(f"Guangxi Dataset ROC Curve")
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.legend()
    plt.savefig(FIG_DIR / "Guangxi_ROC.png", bbox_inches='tight')
    plt.close()
    ...
    plt.title(f"Guangxi Dataset PR Curve")
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.legend()
    plt.savefig(FIG_DIR / "Guangxi_PR.png", bbox_inches='tight')
    plt.close()
    ...
    plt.title("Guangxi Dataset Confusion Matrix")
    plt.ylabel('True')
    plt.xlabel('Predicted')
    plt.savefig(FIG_DIR / "Guangxi_CM.png", bbox_inches='tight')
    plt.close()

    # CSV Predictions
    ids = sorted(list(preds_dict.keys()))
    with open(OUTPUT_DIR / "guangxi_predictions.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    with open(OUTPUT_DIR / "guangxi_error_analysis.txt", "w") as f:
    ...
    # Evaluation Report
    report = "# Guangxi Dataset External Evaluation\n\n"
    for k, v in metrics.items():
        if isinstance(v, float): report += f"**{k}:** {v:.4f}\n"
        elif isinstance(v, list): report += f"**{k}:** {[round(x, 4) for x in v]}\n"
        else: report += f"**{k}:** {v}\n"
    with open(OUTPUT_DIR / "guangxi_report.md", "w") as f: f.write(report)

def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # Point directly to the extracted folder
    data_root = r"data_raw/multimodal_dataset/Construction of a Multimodal Machine Learning Model for Papillary Thyroid Carcinoma Based on Pathomics and Ultrasound Radiomics Data"
    
    ds = GuangxiDataset(data_root)
    print(f"Loaded {len(ds)} samples from Guangxi Dataset (Ultrasound only)")
    
    if len(ds) == 0:
        print("ERROR: No images found. Ensure paths and extraction worked.")
        return
    ...
    for seed in range(5):
        ...
        if not ckpt_path.exists():
            print(f"Skipping Seed {seed}: Checkpoint {ckpt_path} not found.")
            continue
    ...
    print("\n" + "="*50)
    print("GUANGXI EXTERNAL VALIDATION RESULTS")
    print("="*50)
    print(f"Ensemble TTA AUROC : {metrics['AUROC']:.4f}")
    print(f"95% Confidence Int : {metrics['95% CI']}")
    print(f"PR-AUC             : {metrics['PR-AUC']:.4f}")
    print("="*50)
    print(f"Guangxi evaluation complete. Artifacts saved to {OUTPUT_DIR}")
