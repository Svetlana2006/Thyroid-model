"""
Final External Validation Script
Evaluates the frozen 5-seed MultiLevelSwin ensemble on the final unseen external dataset.
"""
...
# ── Final External Dataset Loader ─────────────────────────────────────────────
import cv2

class FinalExternalDataset(Dataset):
    def __init__(self, data_root: str, transforms: list):
        self.data_root = Path(data_root)
        self.transforms = transforms
        self.samples = []
        
        # Load labels from batch1 and batch2
        self.patient_labels = {}
        for batch in ["batch1_image", "batch2_image"]:
            csv_path = self.data_root / batch / f"{batch}_label.csv"
            if csv_path.exists():
                with open(csv_path, "r", encoding="utf-8-sig") as f:
                    reader = csv.DictReader(f)
                    for row in reader:
                        pname = row["patient_name"].strip()
                        label = int(row["histo_label"].strip())
                        self.patient_labels[pname] = label
            
            img_dir = self.data_root / batch / "dataset"
            if img_dir.exists():
                for img_path in img_dir.glob("*.Jpg"):
                    pname = img_path.name.split("_")[0]
                    if pname in self.patient_labels:
                        self.samples.append({
                            "path": str(img_path),
                            "label": self.patient_labels[pname]
                        })
...
def main():
    ...
    parser.add_argument("--data-root", default="data_raw/final_external", help="Path to final external dataset")
    ...
    # Build dataset
    dataset = FinalExternalDataset(args.data_root, TTA_TRANSFORMS)
    print(f"Loaded {len(dataset)} samples from final external dataset.")
    if len(dataset) == 0:
        print("No samples found. Please check dataset extraction and CSV format.")
        return
    ...
    out_file = Path(args.models_dir) / "final_external_results.json"
    with open(out_file, "w") as f:
        json.dump({
            "dataset": "final_external",
            "samples": len(targets),
